...
function renderEmptyState() {
  if (document.getElementById('av-empty-state')) return; // already shown

  const home = document.getElementById('page-home');
  if (!home) return;

  // Hide bin UI so an empty grid doesn't confuse the user
  home.querySelectorAll('.bin-card, .sensor-grid, .mode-selector-wrap, .tb-bin-selector')
      .forEach(el => { el.dataset.esHidden = '1'; el.style.display = 'none'; });

  const es = document.createElement('div');
  es.id = 'av-empty-state';
  es.innerHTML = `
    <div class="av-es-icon">
    ...
    <h2 class="av-es-title">Welcome to AVONIC!</h2>
    <p class="av-es-body">
      You don&rsquo;t have any bins connected yet.<br>
      Claim your physical AVONIC device to start<br>
      monitoring your worms in real&nbsp;time.
    </p>
    <button class="av-es-cta" onclick="openModal('claim-bin-modal')">
      + Claim Your Bin
    </button>
    <p class="av-es-hint">Have a 6-digit claim code? That&rsquo;s all you need.</p>
  `;

  // Inject at the very top of #page-home
  home.prepend(es);
}

function removeEmptyState() {
  const es = document.getElementById('av-empty-state');
  if (!es) return;
  es.remove();
  ...
  const home = document.getElementById('page-home');
  if (home) {
    home.querySelectorAll('[data-es-hidden="1"]').forEach(el => {
      el.style.display = '';
      delete el.dataset.esHidden;
    });
  }
}
...
async function loadDevices() {
  try {
    ...
    if (!devices || devices.length === 0) {
      ...
      S.activeEspID = null;
      renderClaimedBins([]);
      renderEmptyState();   // Phase 3
      return;
    }

    // Bins found — clear any empty state that was showing
    removeEmptyState();
    ...
    maybeStartTour();  // Phase 4 — fires tour if welcome modal armed it
  } catch(e) {
...
/**
 * avonicTourSensorPreview(mode, onNext, onBack)
 * Opens the sensor-detail-modal in demo mode for the onboarding tour.
 * The modal is the self-contained tour step — no Driver.js popover overlaps it.
 * body.av-tour-sensor hides the Driver.js popover + spotlight ring via CSS.
 * Back / Next buttons replace Cancel and call the supplied callbacks.
 */
window.avonicTourSensorPreview = function(mode, onNext, onBack) {
  $('sm-title').textContent = 'Soil Moisture';
  $('sm-icon').src = '/img/monitoring/Sensor Icons/Soil Moisture Icon.svg';
  $('sm-unit').textContent = '%';
  $('sm-val').textContent = '72';

  const statusInd = $('sm-status-ind');
  if (statusInd) {
    const dot = statusInd.querySelector('.dot');
    if (dot) dot.style.background = '#6aab7a';
    const txt = document.getElementById('sm-status-text');
    if (txt) txt.textContent = 'Optimal';
  }
  const wormEl = $('sm-worm-img');
  if (wormEl) wormEl.src = '/img/worm-conditions/Normal.png';

  const manualArea = $('sm-manual-area');
  const actGrid    = $('sm-actuator-grid');

  // Remove any stale tour banner from a previous preview call
  const prev = $('sm-tour-banner');
  if (prev) prev.remove();
    ...
    actGrid.innerHTML = `
      <div class="sm-act-box">
        <div class="sm-act-left">
          <div class="sm-act-icon-bg"><img src="/img/actuators/MistIcon.svg" alt="Mist"></div>
          <div class="sm-act-name">Mist Pump</div>
        </div>
        <label class="tog"><input type="checkbox" disabled><span class="tog-track"></span></label>
      </div>`;
    manualArea.style.display = 'block';

    const banner = document.createElement('div');
    banner.id = 'sm-tour-banner';
    banner.className = 'sm-tour-banner sm-tour-banner--manual';
    banner.innerHTML = `
      <div class="sm-tour-badge-row">
        <span class="sm-tour-badge">🎛️ Manual Mode</span>
        <span class="sm-tour-step-tag">Step 7 / 17</span>
      </div>
      <p>Switch to Manual and actuator toggles appear. Turn the mist pump, fan, or other controls on and off directly from here.</p>`;
    manualArea.insertAdjacentElement('beforebegin', banner);
  } else {
    manualArea.style.display = 'none';

    const readingCard = document.querySelector('#sensor-detail-modal .sm-reading-card');
    const banner = document.createElement('div');
    banner.id = 'sm-tour-banner';
    banner.className = 'sm-tour-banner sm-tour-banner--auto';
    banner.innerHTML = `
      <div class="sm-tour-badge-row">
        <span class="sm-tour-badge">🤖 Auto Mode</span>
        <span class="sm-tour-step-tag">Step 6 / 17</span>
      </div>
      <p>In Auto mode the card shows the live reading and current condition. AVONIC handles everything — no manual controls shown.</p>`;
    if (readingCard) readingCard.insertAdjacentElement('afterend', banner);
  }

  // Replace Cancel button with tour nav buttons.
  // Back is display:none on auto (first modal step) so Next expands full-width.
  const actionsArea = document.querySelector('#sensor-detail-modal .sys-modal-actions');
  if (actionsArea) {
    const backStyle = (mode === 'auto') ? 'style="display:none"' : '';
    actionsArea.innerHTML = `
      <div class="sm-tour-nav">
        <button class="sm-tour-nav-back" id="sm-tour-back" ${backStyle}>← Back</button>
        <button class="sm-tour-nav-next" id="sm-tour-next">Next →</button>
      </div>`;
    const nextBtn = $('sm-tour-next');
    const backBtn = $('sm-tour-back');
    if (nextBtn && onNext) nextBtn.addEventListener('click', onNext);
    if (backBtn && onBack) backBtn.addEventListener('click', onBack);
  }

  // ── Show modal bypassing ModalManager ────────────────────────
  // We CANNOT use openModal() here. ModalManager queues the call silently
  // if _current is already set (Back→Auto→Manual cycles leave stale state).
  // We also CANNOT use closeAllModals() because that's patched and restores
  // the Cancel button, fighting our tour buttons. Reset state directly.
  ModalManager.closeAll();                          // reset _current + _queue
  const modal   = $('sensor-detail-modal');
  const overlay = $('sys-modal-overlay');
  if (overlay) overlay.classList.add('show', 'tour-preview');
  if (modal)   modal.classList.add('show', 'tour-preview');
  document.body.classList.add('av-tour-sensor');
};

// Patch closeAllModals to strip all tour state whenever modal closes
(function() {
  const _orig = window.closeAllModals;
  window.closeAllModals = function() {
    const modal   = $('sensor-detail-modal');
    const overlay = $('sys-modal-overlay');
    if (modal)   modal.classList.remove('tour-preview');
    if (overlay) overlay.classList.remove('tour-preview');
    // Remove body flag so Driver.js popover + spotlight resume
    document.body.classList.remove('av-tour-sensor');
    // Remove tour banner and restore the standard Cancel button
    const banner = $('sm-tour-banner');
    if (banner) banner.remove();
    const actionsArea = document.querySelector('#sensor-detail-modal .sys-modal-actions');
    if (actionsArea && !actionsArea.querySelector('.btn-cancel')) {
      actionsArea.innerHTML = `<button class="btn-cancel w-100" onclick="closeAllModals()">Cancel</button>`;
    }
...
async function handleClaimBin() {
  ...
  try {
    ...
    await loadDevices(); // Refresh device list — maybeStartTour() fires inside loadDevices
    closeTopModal();
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  (function parseTabParam() {
    ...
    if (tab !== 'register' && tab !== 'login') return;

    const trySwitch = setInterval(function () {
...
window.avonicStartTour = function () {
  ...
  const NAV = 600;
  const SUB = (n) => `<span style="opacity:.45;font-size:11px;font-weight:400">${n} / 17</span>`;
  ...
  function startBFTour() {
    const d5 = window.driver.js.driver({
      ...
      onNextClick() {
        if (d5.isLastStep()) { tourCleanup(); d5.destroy(); }
        else d5.moveNext();
      },
      steps: [
    ...
    const d2 = window.driver.js.driver({
      ...
      onNextClick() {
        ...
        if (idx === 0) {
          ...
          // Pause d2 here — launch Auto→Manual modal sub-tour.
          // When Manual's Next fires we DESTROY d2 and spin up startModeTour
          // instead of calling d2.moveNext(). This avoids any state resurrection
          // across the modal boundary and eliminates the 320ms gap where the
          // overlay's onclick="closeAllModals()" could nuke the tour.
          function showAuto() {
          ...
          function showManual() {
            window.avonicTourSensorPreview(
              ...
              function() {
                ...
                document.body.classList.remove('av-tour-sensor');
                const m = $('sensor-detail-modal');
                const o = $('sys-modal-overlay');
...
// ════════════════════════════════════
// PHASE 5 — Contextual Tooltip System
// ════════════════════════════════════
(function initTooltips() {
  function setup() {
    document.querySelectorAll('.av-help').forEach(function (tip) {

      // Auto-flip if the element is near the top of the viewport
      var rect = tip.getBoundingClientRect();
      if (rect.top < 140) tip.classList.add('av-tip-flip');

      // Tap/click toggle — closes others, opens this one
      tip.addEventListener('click', function (e) {
        e.stopPropagation();
        var isOpen = tip.classList.contains('av-tip-open');
        document.querySelectorAll('.av-help.av-tip-open')
                .forEach(function (t) { t.classList.remove('av-tip-open'); });
        if (!isOpen) tip.classList.add('av-tip-open');
      });
    });
    ...
    // Re-check flip direction on scroll (settings panel scrolls)
    document.addEventListener('scroll', function () {
      document.querySelectorAll('.av-help').forEach(function (tip) {
        var rect = tip.getBoundingClientRect();
        tip.classList.toggle('av-tip-flip', rect.top < 140);
      });
    }, { passive: true });
  }
